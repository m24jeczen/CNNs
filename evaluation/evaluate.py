...
def save_evaluation_results(model_name, num_epochs, augmentations, batch_size, weight_decay, min_lr, max_lr, scheduler_type, test_metrics):
    """
    Appends evaluation results to 'test_results.txt' inside 'Experiments' directory.
    """    
    augmentations_str = "_".join(augmentations) if augmentations else "none"
    model_info = f"{model_name}_{num_epochs}_{augmentations_str}_{batch_size}_{weight_decay}_{min_lr}_{max_lr}_{scheduler_type}"
...
def evaluate_model(model, test_loader, num_epochs, weight_decay=0, min_lr=0.0001, max_lr=0.001, scheduler_type="none", device=None, augmentations=None):
    """
    Evaluates a PyTorch model and logs results.
    """
    if device is None:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
